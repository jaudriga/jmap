...
public class Session {
    ...
    public HttpUrl getDownloadUrl(String accountId, String blobId, String name, String type) {
        ...
        if (builder == null) {
            throw new IllegalStateException(
                    String.format("Unable to assemble final download Url from base=%s and downloadUrl=%s", base, downloadUrl)
            );
        }
        return builder.build();
    }
    ...
    public HttpUrl getEventSourceUrl(Collection<Class<?extends AbstractIdentifiableEntity>> types, CloseAfter closeAfter, Long ping) {
        ...
        if (builder == null) {
            throw new IllegalStateException(
                    String.format("Unable to assemble final eventSource Url from base=%s and eventSourceUrl=%s", base, eventSourceUrl)
            );
        }
        return builder.build();
    }
